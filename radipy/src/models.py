...
class gEUD(__model__):
    ...
    def __init__(self, v: np.ndarray, D: np.ndarray, a: float):
...
class LKB(__model__):
    ...
    def __init__(self, D50: float, m: float, gEUD_model: gEUD):
        super().__init__(locals())
        self.val = self.compute()

    def compute(self):
        t = (1 / self.m) * (self.gEUD_model.val / self.D50 - 1) 
        res = quad(lambda u: np.exp(-(u**2)/2), np.NINF, t)
        return res
...
class RS(__model__):
    ...
    def compute(self):
        ...
        prod = np.prod(np.power(np.power(1 - self.PDi, self.seriality), vol_proportion))
